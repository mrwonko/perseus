...
\chapter{Implementation}
	...
	\section{Semantic analysis}
		...
		Of particular interest are \lstinline$boost::variant$ nodes, since they represent one of multiple possible types. They are processed using a visitor as outlined in listing \ref{lst:visitor}; types are distinguished through overloading.
		...
		These transforming functions return a different node type because the transformed syntax tree has different nodes; one example are operations: Initially they're in a flat list (see listing \ref{lst:expression}) but for proper code generation binary operations are required. This leads to different nodes, which also have additional annotations like types.
